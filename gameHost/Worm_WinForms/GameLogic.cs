    ...
    public partial class WormGame
    {
        ...
        private void UpdateWorm(object sender, EventArgs eventArgs)
        {
            ...
            for (var i = _worm.Count - 1; i >= 0; i--)
            {
                if (i == 0)
                {
                    ...
                    //if ((head.X != _food.X) || (head.Y != _food.Y)) continue;       /// TODO еда равна null
        ...
        public void ConnectToServer()
        {
            try
            {
                ...
                var dataString = "X" + _worm[0].X + "Y" + _worm[0].Y + "direction" + Direction;
                var data = Encoding.UTF8.GetBytes(dataString);
                _stream = _client.GetStream();
                _stream.Write(data, 0, data.Length);
                MessageBox.Show("Отправлено серверу: " + Encoding.Default.GetString(data));

                //_food.X = 10;         /// TODO еда должна генерироваться на сервере
                //_food.Y = 10;
